...
/**
 * Enforces tenant isolation by comparing the request's tenant context against a resource's tenant
 * ID.
 *
 * <p>WHY a utility class: every service that accesses tenant-scoped resources must verify that the
 * authenticated user belongs to the same tenant. Fail-fast with {@link TenantMismatchException}
