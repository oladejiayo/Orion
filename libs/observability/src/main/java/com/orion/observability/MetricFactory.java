 ...
 *
 * <p>Every metric created through this factory automatically includes a {@code tenant} tag sourced
 * from the current {@link CorrelationContextHolder}. Additional tags can be supplied per metric.
 *
 * <p>This class wraps {@link MeterRegistry} to enforce consistent metric naming and tenant-level
 * segmentation across all Orion services.
 */
public final class MetricFactory {
    ...
    public Timer timer(String name, String description, String... tags) {
        return Timer.builder(name).description(description).tags(baseTags(tags)).register(registry);
    }
    ...
    /**
     * Combines service tag with any additional tags. The tenant tag is added as a common tag so it
     * appears on every metric from this service.
     */
